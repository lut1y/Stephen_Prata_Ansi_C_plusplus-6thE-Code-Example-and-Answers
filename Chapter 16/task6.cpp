...
int task6() {
    ...
    int qs;
    cin >> qs;
    std::vector<Item> line; // очередь может содержать до qs людей
...
// Запуск моделирования
    for (int cycle = 0; cycle < cyclelimit; cycle++)
    {
        ...
        {
            if (line.size() == qs) // если полная очередь
                turnaways++;   // не допущен
            else
            {
                customers++;		
                temp.set(cycle);    // cycle = время прибытия
                line.push_back(temp); // добавление новичка в очередь
            }
        }
        if (wait_time <= 0 && line.size() != 0) { // если у клиент обслужен (или тот отсутствует) и очередь есть
            line.erase(line.begin(), line.begin()+1);      // обслуживание следующего клиента
